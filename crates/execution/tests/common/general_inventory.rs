//! Inventory composition through the signed general-call authorization table.
//!
//! Warehouse constructors: 1 AdminCap (), 2 Stock (sku, available),
//! 3 Reservation (order, sku, quantity), 4 Shipment (order, sku, quantity).
//! Entrypoints: init((sku, available), []), reserve((order, quantity),
//! [AdminCap Read, Stock Write]), fulfil((), [AdminCap Read,
//! Reservation Consume, Policy Write]), transfer(Bytes32, [Shipment Write]).
//! Only fulfil forwards the Policy handle; the warehouse never reads/writes it.
//!
//! Independent policy constructors: 1 Policy (limit, total_approved),
//! 2 ApprovalRecord (quantity, total_after). configure(U64 limit, []) is its
//! initializer; approve(U64 quantity, [Policy Write]) accounts and creates an
//! approval BEFORE checking the limit, deliberately exercising atomic rollback.
//! All objects are owned by get_caller (the transaction sender). Arguments,
//! bodies and type tags use the existing canonical encoders.
//!
//! The authorization index selects signed caller/callee targets, entrypoint and
//! object selectors; this fixture never encodes authority or chooses a target.
//! The same fulfil code works for a same-instance library target when that
//! instance already has valid policy state and the signed table permits it.
...
(import "sunrise" "call_contract" (func $call (param i32 i32 i32 i32 i32) (result i32)))
...
pub fn warehouse(
    ...
    policy_origin: &PackageOrigin,
    authorization_index: u32,
) -> InventoryPackage {
    assert!(i32::try_from(authorization_index).is_ok());
            ...
            entrypoints: vec![
                entry(
                    ...
                    vec![
                        ...
                        param(policy_origin, 1, ObjectMode::Write),
                    ],
                ),
                entry("init", vec![]),
                entry(
 ...
 (func (export "fulfil") (local $n i32)
  ...
  (call $zero (call $call (i32.const {authorization_index}) (i32.const 512) (i32.const 1) (i32.const 19456) (i32.const {scalar_len})))
...
/// Independently initialized dispatch policy with observable callee-scope effects.
pub fn dispatch_policy(origin: &PackageOrigin) -> InventoryPackage {
    let abi: CallAbi = CallAbi {
        objects: PackageAbi {
            ...
            entrypoints: vec![
                entry("approve", vec![param(origin, 1, ObjectMode::Write)]),
                entry("configure", vec![]),
            ],
        },
        arguments: vec![ValueLayout::U64, ValueLayout::U64],
        bodies: vec![tuple_layout(2), tuple_layout(2)],
    };
    let policy_tag: Vec<u8> = tag(origin, 1);
    let approval_tag: Vec<u8> = tag(origin, 2);
    let body: Vec<u8> = tuple_arguments(&[0, 0]);
    let segments: String = [
        data(1024, &policy_tag),
        data(2048, &approval_tag),
        data(16384, &body),
        data(17408, &body),
    ]
    .concat();
 ...
 (func (export "configure") (local $q i64)
  ...
  (i64.store (call $slot (i32.const 16384) (i32.const {body_len}) (i32.const 0)) (local.get $q))
  (drop (call $create (i32.const 1024) (i32.const {policy_tag_len}) (i32.const 256) (i32.const 16384) (i32.const {body_len}))))
 (func (export "approve") (local $n i32) (local $q i64) (local $limit i64) (local $old i64) (local $total i64)
  (call $prepare (i32.const 1)) (local.set $n (call $object (i32.const 0)))
  (local.set $q (i64.load (call $scalar (i32.const 8192) (global.get $al))))
  (call $require (i64.gt_u (local.get $q) (i64.const 0)))
  (local.set $limit (i64.load (call $slot (i32.const 12288) (local.get $n) (i32.const 0))))
  (local.set $old (i64.load (call $slot (i32.const 12288) (local.get $n) (i32.const 1))))
  (local.set $total (i64.add (local.get $old) (local.get $q)))
  (call $require (i64.ge_u (local.get $total) (local.get $old)))
  (i64.store (call $slot (i32.const 12288) (local.get $n) (i32.const 1)) (local.get $total))
  (call $zero (call $write (i32.const 0) (i32.const 12288) (local.get $n)))
  (i64.store (call $slot (i32.const 17408) (i32.const {body_len}) (i32.const 0)) (local.get $q))
  (i64.store (call $slot (i32.const 17408) (i32.const {body_len}) (i32.const 1)) (local.get $total))
  (drop (call $create (i32.const 2048) (i32.const {approval_tag_len}) (i32.const 256) (i32.const 17408) (i32.const {body_len})))
  (call $zero (call $event (i32.const 2048) (i32.const {approval_tag_len}) (i32.const 17408) (i32.const {body_len})))
  ;; Intentionally late: an over-limit dispatch must discard both scopes' effects.
  (call $require (i64.le_u (local.get $q) (local.get $limit)))))"#,
        body_len = body.len(),
        policy_tag_len = policy_tag.len(),
        approval_tag_len = approval_tag.len(),
    );
    let wasm: Vec<u8> = wat::parse_str(&wat).unwrap();
    InventoryPackage {
        wat,
        wasm,
        abi,
        initializer: Some("configure".to_owned()),
