...
slice 3, equivocation evidence, design accepted but not implemented in
...
slice 2's design (epoch transition); DR-0133 fixes slice 3's detailed design
(equivocation evidence), but its implementation, slice 4's detailed design
and implementation, and phase 3 economics/security completion still require
their complete repository gates and fresh reviews before being claimed done.
Contract
...
- [ ] **Phase 2 — validator lifecycle
  ...
  [DR-0132](docs/architecture/decisions/0132-fastvote-epoch-transition.md);
  Slice 3's detailed design is accepted but not implemented per
  [DR-0133](docs/architecture/decisions/0133-fastvote-equivocation-evidence.md),
  and only Slice 4's detailed wire/API decision remains pending).**
  ...
  - [ ] **Slice 3 — equivocation evidence (detailed design accepted,
    not implemented, in
    [DR-0133](docs/architecture/decisions/0133-fastvote-equivocation-evidence.md)).**
    Explicit canonical equivocation evidence covering same-transaction
    conflicting-outcome, cross-transaction same-object-version (via an
    in-place `FastVote`/`FastCertificate` `locked_objects_digest`
    extension, no v2/compat layer), and epoch-transition conflicting-target
    misconduct, normalized for later phase 3 slashing; no economics
