...
**Phase 3 functional delivery order (per [DR-0147](docs/architecture/decisions/0147-function-first-network-delivery.md)):**
functional work is prioritized ahead of load testing; peak TPS, concurrent-user
and recovery targets remain undecided and are deferred to post-launch
hardening. The active order is:
...
- [ ] bounded independent-validator functional start/restart/replay/
  ...
  and the focused security review, before exposing this ingress.

Representative sustained load/soak/capacity certification and adopting
throughput/recovery SLOs are post-launch hardening, not prerequisites for
this order, Phase 3 functional closure or initial network startup. No target
numbers are adopted.
...
- [ ] **Phase 3 — economics/security completion.** Architecture is split into
  ...
  - [ ] **Slice 2 — closed release authority and economics
    ...
    - [ ] certified fee escrow and claims:
      ...
      - [ ] capacity/load/soak certification for concurrent escrows, claim
        ...
        functional-closure prerequisite; no target numbers are adopted;
  ...
  **Remaining Phase 3 completion:** per
  [DR-0147](docs/architecture/decisions/0147-function-first-network-delivery.md),
  functional closure needs authenticated external validator request/event-
  driven ingress for prepare/certificate/apply, a CLI end-to-end quorum
  submission path, the already-implemented bond/epoch/equivocation/reward/
  claim lifecycle exposed through explicit authenticated operator/network
  surfaces where needed, bounded independent-validator functional start/
  restart/replay/authorization evidence, a documented deployment/
  configuration walkthrough, and the Phase 3 security and tech-lead review
  gate. Declaring an initial network load/recovery target and establishing
  representative sustained claim/restart-sweep capacity move to post-launch
  hardening; they are not a Phase 3 functional prerequisite, and no target
  numbers are adopted yet. The bounded DR-0145/0146 regressions remain
  fixture-level evidence, not that certification. Protocol-version activation
  is a separately blocked future gate; there is no live version-switch path to
  exercise in this phase. Revisit it before implementing that path. FastVote
  is not complete until this phase closes.
...
- [ ] Post-launch hardening (per
  ...
  trials. Not required for Phase 3 functional closure or initial network
...
- [ ] Post-launch hardening (per
  ...
  domains. This item is not a Phase 3 functional-closure prerequisite; the
