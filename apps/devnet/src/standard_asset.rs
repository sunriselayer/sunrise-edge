...
//! [`standard_assets::derive_asset_id`] — there is no `Create` entrypoint yet,
//! so this is the only place that `AssetId` comes from (see DR-0107).
...
/// Only entrypoint exposed by the local devnet Standard Asset v1 module.
...
/// `Create` entrypoint, so this address never signs a transaction; it exists
/// purely as a stable derivation input for [`derive_devnet_asset_id`].
...
mod tests {
    ...
    use standard_assets::{StandardAssetTransferArgsV1, encode_standard_asset_transfer_args_v1};
